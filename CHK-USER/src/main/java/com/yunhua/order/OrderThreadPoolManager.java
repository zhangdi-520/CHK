 ...
 */
@Component
public class OrderThreadPoolManager implements BeanFactoryAware {
    ...
    final RejectedExecutionHandler handler = new RejectedExecutionHandler() {
        ...
        public void rejectedExecution(Runnable r, ThreadPoolExecutor executor) {
            //订单加入到缓冲队列
            msgQueue.offer(((OrderThread) r).getAcceptStr());
            System.out.println("系统任务太忙了,把此订单交给(调度线程池)逐一处理，订单号：" + ((OrderThread) r).getAcceptStr());
        }
    };
    ...
    public void addOrders(String orderId){
        ...
        if (cacheMap.get(orderId) == null) {
            ...
            OrderThread businessThread = new OrderThread(orderId);
